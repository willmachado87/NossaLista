...
export class AddListaPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, 
      ...
      this.editList = navParams.get("editList");
      this.idLista = navParams.get("idLista");
      //console.log("idlista: ",this.idLista);         
      ...
      if(this.editList == false || undefined){
        ...
        this.lista.subscribe(dataFs =>{
          this.lista2 = dataFs;        
          dataFs.usuarios.forEach((date, i) => 
          setTimeout(() => {
            let temp = this.bd.doc('usuarios/'+date).valueChanges();
            temp.subscribe(w =>{
              this.usuariosFull.push(w);              
            })
          }, 100 * i++));          
        console.log("full:", this.usuariosFull); 
      })
    }  
